 ...
 */
export async function runSamplePass(component: string, basename: string, output: Array<Record<number | string | symbol, any> | number | string>) {
	...
	// We used to check if file exists using `vscode.FileSystem.stat`.
	// But because we override data anyways, just fetch snapshot JSON.
	try {
		expectedJson = await loadJsonFile<PartialJsonObject>(data);
	} catch (_) {
		/* swallow exception and delegate to assert */
	}
