...
import { loadJsonFile, TextmateScopeSelector, TextmateScopeSelectorMap } from '../../util/common';
...
async function scopeInput() {
	const data = getComponentSampleDataUri('selectors', 'selector');
...
async function mapInput() {
	const data = getComponentSampleDataUri('selectors', 'map');
